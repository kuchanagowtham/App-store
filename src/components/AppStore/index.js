...
class AppStore extends Component {
  state = {getid: tabsList[0].tabId, searchInput: ''}

  getActivetabId = tabId => {
    this.setState({getid: tabId})
  }
  ...
  render() {
    const {getid, searchInput} = this.state
    const filteredApps = appsList.filter(eachApp => eachApp.category === getid)
    const searchedResults = filteredApps.filter(each =>
        ...
        <div className="app-store">
          ...
          <div className="input-container">
            <input
              type="search"
              placeholder="Search"
              onChange={this.getSearchDetails}
              className="input-box"
            />
            <img
          ...
          <ul className="tab-item">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                key={eachTab.tabId}
                isActive={getid === eachTab.tabId}
              />
            ))}
          </ul>
          <ul className="app-container">
            {searchedResults.map(eachList => (
              <AppItem itemsList={eachList} key={eachList.appId} />
            ))}
          </ul>
        </div>
      </div>
